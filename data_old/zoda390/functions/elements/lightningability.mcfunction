...
tag @s remove next
tag @s add last
execute as @e[type=minecraft:zombie,tag=!hitByLightning,limit=1,sort=nearest,distance=..10] at @s run function zoda390:elements/findnextzombie
...
execute as @e[tag=L2] at @e[tag=L1] facing entity @s feet run function zoda390:elements/drawline
kill @e[tag=lightning]

execute as @e[type=zombie,tag=next] at @s run function zoda390:elements/lightningability